...
const propTypes = {
  ...
  sortColumn: PropTypes.oneOfType([PropTypes.string, PropTypes.object]),
};
...
function  SortTableVis(element, props) {
  const {
    data,
    height,
  ...
  $container.addClass('superset-legacy-chart-sort-table');
  ...
  const limitMetric = Array.isArray(timeseriesLimitMetric)
    ...
    : timeseriesLimitMetric;
  const limitColumn = Array.isArray(sortColumn)
    ? sortColumn[0]
    : sortColumn;
  if (limitMetric || limitColumn) {
    // Sort by as specified
    sortBy = limitColumn.label || limitColumn || limitMetric.label || limitMetric;
  ...
  if (sortBy) {
    ...
    if (metrics.indexOf(sortBy) < 0 && !limitColumn) {
...
SortTableVis.displayName = 'SortTableVis';
SortTableVis.propTypes = propTypes;

export default SortTableVis;
