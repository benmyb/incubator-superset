...
class EchartsStackViz(BaseViz):
    ...
    def run_extra_queries(self):
        ...
        df2 = df2.pivot_table(
            ...
            fill_value=0.0,
            values=utils.get_metric_names(fd.get('metrics')))
        df2.rename(columns={fd.get('metrics')[0]: '#'}, inplace=True)
        self._extra_chart_data.extend(self.to_series(df2))
    ...
    def get_data(self, df):
        ...
        col = '#'
        for dd in d:
            ddf = pd.DataFrame(dd['values'])
            col_name = ', '.join(dd['key'])
            series_data[col_name] = ddf['y']
            if is_all:
                is_all = False
            else:
                series_data[col] = series_data[col].sub(ddf['y'], fill_value=0)
        ...
        if dont_show_other:
            series_data.drop(columns=['#'], inplace=True)
        ...
        if dont_show_other:
            ...
            series_data.rename(columns={'#': 'other', 'x': 'timestamp'}, inplace=True)
    ...
    def get_data(self, df):
        ...
        for dd in d:
            ...
            if is_all:
                ...
                series_data[col] = series_data[col].sub(ddf['y'], fill_value=0)
        series_data.set_index(['x'], inplace=True)
        if len(self.form_data.get('groupby')) == 1 and self.form_data.get('adhoc_filters'):
            for filter_values in self.form_data.get('adhoc_filters'):
                if filter_values['subject'] == self.form_data.get('groupby')[0]:
                    col_seq = filter_values['comparator'] + ['#']
                    series_data = series_data[col_seq[::-1]]
                    break
        dont_show_other = not self.form_data.get('show_other') or (self.form_data.get('limit') or 10000) + 1 > len(series_data.columns.values.tolist())
        if dont_show_other:
            series_data.drop(columns=['#'], inplace=True)
...
class HighChartStackViz(BaseViz):
    """ HighCharts Stack Chart"""
    viz_type = 'highcharts_stack'
    verbose_name = _('HighChart Stack View')
    is_timeseries = True
    ...
    def query_obj(self):
        d = super(HighChartStackViz, self).query_obj()
        fd = self.form_data
        ...
        if len(fd.get('groupby')) > 1:
            raise Exception(_(
                'Group by or thumbnail more than one column'))
    ...
    def get_data(self, df):
        ...
        for dd in d:
            ...
            if is_all:
                ...
                series_data[col] = series_data[col].sub(ddf['y'], fill_value=0)
        series_data.set_index(['x'], inplace=True)
        series_data = series_data[series_data.apply(sum).sort_values(ascending=False).keys().tolist()]
        series_data.reset_index(0, inplace=True)
        series_data = series_data.fillna(0)
        if self.form_data('limit') + 2 > len(series_data.columns.tolist()):
            series_data.drop(columns=['#'], inplace=True)
            series_data.rename(columns={'x': 'timestamp'}, inplace=True)
        else:
            series_data.rename(columns={'#': 'other', 'x': 'timestamp'}, inplace=True)
        d_list = dict()
        col_list = series_data.columns[1:].tolist()
        for col in col_list:
            d_list[col] = series_data[['timestamp', col]].values.tolist()
        fd = self.form_data
        title = fd.get('groupby')[0] + ' - ' + fd.get('metrics')[0] + ' - top' + str(fd.get('limit'))
        return dict(
            title=title,
            names=col_list,
            raw_data=d_list,
...
class EchartsMultiLineViz(BaseViz):
    """ MultiLine Chart"""
    viz_type = 'echarts_multiline'
    verbose_name = _('MultiLine View')
    sort_series = False
    is_timeseries = True
    ...
    def query_obj(self):
        d = super(EchartsMultiLineViz, self).query_obj()
        fd = self.form_data
        m1 = fd.get('metrics')
    ...
    def get_data(self, df):
        ...
        for series in chart_data:
            ...
            if 'classed' in series:
                metric += ',' + series['key'][2]
            tmp_values = series['values']
            for dataxy in tmp_values:
                tmp = [dataxy['x'], dataxy['y']]
                values.append(tmp)
        ...
        return dict(
            metrics=metrics,
            metrics_2=fd.get('percent_metrics'),
